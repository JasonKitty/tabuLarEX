...
from nougat.latex2html1 import latex2html

def teds(gt, pred):
    gt_html = latex2html(gt)
    pred_html = latex2html(pred)
    if not gt_html:
        # print("GT LaTeX to HTML conversion failed.")
        return -1
    if not pred_html:
        # print("Prediction LaTeX to HTML conversion failed.")
        return -1
    compare = TEDS()
    teds_score = compare(gt_html, pred_html)
    return teds_score
...
def compute_metrics(pred, gt, minlen=4):
    ...
    metrics["bleu"] = nltk.translate.bleu_score.sentence_bleu(
